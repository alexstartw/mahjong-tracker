...
async function getPlayerProfile(id: string) {
  ...
  return {
    ...
    wins, losses, draws,
    winRate: player.sessions.length > 0 ? Math.round((wins / player.sessions.length) * 100) : 0,
    bestWin:   player.sessions.length > 0 ? Math.max(...player.sessions.map((sp) => sp.amount)) : 0,
    worstLoss: player.sessions.length > 0 ? Math.min(...player.sessions.map((sp) => sp.amount)) : 0,
...
export default async function PlayerProfilePage({ params }: Props) {
  ...
  const totalColor = isPos ? "var(--win)" : isNeg ? "var(--loss)" : "var(--muted-foreground)";
    ...
    <div className="min-h-screen" style={{ background: "var(--background)" }}>
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 space-y-5">
        ...
        <div className="card p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="text-right">
              <p className="text-3xl font-semibold tabular" style={{ color: totalColor }}>
                {isPos ? "+" : ""}{player.totalAmount.toLocaleString()}
              </p>
              <p className="text-xs mt-0.5" style={{ color: "var(--muted-foreground)" }}>累積輸贏</p>
            </div>
          </div>
          ...
          <div className="mt-5 pt-5" style={{ borderTop: "1px solid var(--border)" }}>
            <div className="grid grid-cols-3 text-center gap-2">
              {[
                ...
                { label: "平", value: player.draws,  color: "var(--muted-foreground)" },
                { label: "輸", value: player.losses, color: "var(--loss)" },
              ].map((s) => (
                <div key={s.label}>
                  <p className="text-xl font-semibold tabular" style={{ color: s.color }}>{s.value}</p>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { label: "總場次", value: player.sessionCount, suffix: "場", color: "var(--foreground)" },
            { label: "勝率",   value: player.winRate,      suffix: "%",  color: "var(--foreground)" },
            ...
            <div key={s.label} className="card p-4 text-center">
              <p className="text-xl font-semibold tabular" style={{ color: s.color }}>
                {s.value}{"suffix" in s && <span className="text-sm font-normal ml-0.5" style={{ color: "var(--muted-foreground)" }}>{s.suffix}</span>}
              </p>
        ...
        {player.recentSessions.length > 0 && (
          <div className="card overflow-hidden">
            <div className="px-5 py-3.5" style={{ borderBottom: "1px solid var(--border)" }}>
              <p className="text-sm font-medium" style={{ color: "var(--foreground)" }}>近期戰績</p>
            </div>
            <div>
              {player.recentSessions.map((s, i) => {
                ...
                return (
                  <div
                    ...
                    style={{ borderBottom: i < player.recentSessions.length - 1 ? "1px solid var(--border)" : "none" }}
                  >
                    <div className="flex items-center gap-3">
                      <div className="text-center w-8">
                        <p className="text-sm font-medium tabular leading-none" style={{ color: "var(--foreground)" }}>
                      ...
                      <div>
                        <p className="text-sm font-medium" style={{ color: "var(--foreground)" }}>{s.venue}</p>
                        <p className="text-xs mt-0.5" style={{ color: "var(--muted-foreground)" }}>{s.stakes}</p>
                      </div>
                    </div>
                    <p
                      className="text-base font-semibold tabular"
                      style={{ color: isWin ? "var(--win)" : isLoss ? "var(--loss)" : "var(--muted-foreground)" }}
                    >
