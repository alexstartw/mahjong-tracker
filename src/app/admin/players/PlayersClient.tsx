  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="card p-5">
        <p className="text-xs font-medium uppercase tracking-wider mb-4" style={{ color: "var(--muted-foreground)" }}>
          新增玩家
        </p>
        <form onSubmit={handleAdd} className="flex items-center gap-3">
          ...
          <label className="flex items-center gap-2 cursor-pointer shrink-0">
            <button
              ...
              style={{ background: isGuest ? "var(--primary)" : "var(--muted)", border: "1px solid var(--border)" }}
            >
              <span
        ...
        {error && (
          <p className="text-xs mt-3 px-3 py-1.5 rounded" style={{ color: "var(--destructive)", background: "color-mix(in srgb, var(--destructive) 10%, transparent)" }}>
            {error}
          </p>
        )}
      </div>
...
function PlayerTable({
  ...
  return (
    <div className="card overflow-hidden">
      ...
      {players.length === 0 ? (
        <p className="px-5 py-8 text-sm text-center" style={{ color: "var(--muted-foreground)" }}>尚無玩家</p>
      ) : (
        <div>
          {players.map((p, i) => {
            ...
            return (
              ...
              >
                <Link href={`/players/${p.id}`} className="flex-1 min-w-0 hover:opacity-70 transition-opacity">
                ...
                <span
                  ...
                  style={{ color: isPos ? "var(--win)" : isNeg ? "var(--loss)" : "var(--muted-foreground)" }}
                >
                  {isPos ? "+" : ""}{p.stats.totalAmount.toLocaleString()}
                </span>
